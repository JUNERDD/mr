...
import { type WorktreeDirSettings, worktreeDirSourceText } from './worktree-dir.js'

export function printSettings(
  settings: MrSettings,
  detached: DetachedSettings,
  requestCommand: RequestCommandSettings,
  worktreeDir: WorktreeDirSettings,
  ...
  context.ui.panel('mr 设置', [
    ...
    `worktree 目录: ${worktreeDir.effective} (${worktreeDirSourceText(worktreeDir.source)})`,
    ...
    `当前仓库 worktree 目录: ${worktreeDir.local ?? (worktreeDir.localAvailable ? '未设置' : '不可用')}`,
    ...
    `全局用户 worktree 目录: ${worktreeDir.global ?? '未设置'}`,
